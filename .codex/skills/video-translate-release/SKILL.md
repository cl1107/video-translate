...
仓库的 Landing 页面由 `.github/workflows/pages.yml` 构建并部署到 GitHub Pages。workflow 使用 `pnpm/action-setup@v6.0.8` 和显式的 `pnpm@11.12.0` 普通 npm 包安装，避免浮动 bootstrap 与 standalone 自更新路径产生安装错误。首次启用时，在仓库 Settings → Pages 将 Source 设为 GitHub Actions；之后每次推送 `main` 会自动部署 `apps/landing`。
