...
export class SegmentMerger {
  ...
  mergeSegments(
    ...
    switch (strategy) {
      ...
      case MergeStrategy.PUNCTUATION_BASED:
        processedSegments = this.mergeByPunctuation(
          processedSegments,
          mergedOptions
        )
        break
    ...
    for (const segment of segments) {
      ...
      if (
        ...
        currentSegment.confidence =
          (currentSegment.confidence + segment.confidence) / 2
      } else {
        // 检查是否满足最小时长要求
        if (
  ...
  private mergeByPunctuation(
    ...
    const sentenceEndings =
      options.punctuationMode === 'strict' ? /[。！？.!?]/ : /[。！？.!?，,、]/
  ...
  private mergeByLength(
    ...
    for (const segment of segments) {
      ...
      const combinedLength =
        currentSegment.originalText.length + segment.originalText.length
      const timeGap = segment.start - currentSegment.end
