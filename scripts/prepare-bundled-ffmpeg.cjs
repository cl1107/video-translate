...
async function verify(ffmpeg, ffprobe) {
  ...
  if (!/\bsubtitles\b/.test(output)) {
    throw new Error('Downloaded FFmpeg does not include the required subtitles/libass filter')
  }
}
...
async function main() {
  ...
  const source = platformSource?.[process.platform === 'darwin' ? 'universal' : requestedArch]
  if (!source) {
    throw new Error(
  ...
  const ffmpegDestination = path.join(destination, `ffmpeg${executableExtension}`)
  const ffprobeDestination = path.join(destination, `ffprobe${executableExtension}`)
  ...
  try {
    for (const [index, url] of source.urls.entries()) {
      ...
      const archive = path.join(tempDirectory, source.archive || `archive-${index}${extension}`)
    ...
    const ffmpegSource = await findFile(tempDirectory, `ffmpeg${executableExtension}`)
    const ffprobeSource = await findFile(tempDirectory, `ffprobe${executableExtension}`)
    if (!ffmpegSource || !ffprobeSource) {
      throw new Error('Downloaded archive did not contain both ffmpeg and ffprobe')
    }
